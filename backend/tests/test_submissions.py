...
async def test_create_submission_success_and_versioning(client, db_session):
    """Testa upload de submissão com sucesso por membro, com task_title e incremento de versão."""
    ...
    # 3. Aluno 1 (membro) envia arquivo 1 com task_title
    ...
    resp = await client.post(
        "/api/submissions",
        data={"project_id": str(project.id), "task_title": "Tarefa Inicial"},
        files=file_data
    )
    ...
    assert sub_data["original_filename"] == "document_v1.pdf"
    assert sub_data["task_title"] == "Tarefa Inicial"
    assert sub_data["status"] == "pending"
    assert sub_data["uploader_id"] == str(student1.id)

    # 4. Aluno 1 envia arquivo 2 (versão 2) sem task_title
    ...
    assert sub_data_v2["original_filename"] == "document_v2.pdf"
    assert sub_data_v2["task_title"] is None
...
async def test_list_submissions_history(client, db_session):
    ...
    sub1 = Submission(project_id=project.id, version=1, file_path="uploads/mock_1.pdf", filename="mock_1.pdf", original_filename="mock_1.pdf", uploader_id=student.id, status="pending")
    sub2 = Submission(project_id=project.id, version=2, file_path="uploads/mock_2.pdf", filename="mock_2.pdf", original_filename="mock_2.pdf", uploader_id=student.id, status="pending")
