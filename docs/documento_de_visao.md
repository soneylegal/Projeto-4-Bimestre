...
Abaixo estão listadas as 5 histórias de usuário principais do sistema, detalhando seus fluxos de execução e critérios de aceitação específicos:

### US001 — Login Unificado via SUAP (BFF)
* **Como** usuário (aluno, orientador, coordenador ou administrador) do IFAL
* **Eu quero** fazer login na plataforma utilizando minhas credenciais do SUAP
* **Para que** eu possa acessar minhas informações de forma segura sem precisar memorizar uma nova senha.
* **Fluxo de Usuário:**
  1. O usuário entra no site do IFAL Projetos.
  2. Caso não esteja autenticado, é apresentado à tela de login com o botão "Entrar com SUAP".
  3. Ao clicar no botão, é redirecionado para a tela oficial de login do SUAP.
  4. O usuário insere sua matrícula/credencial e senha do SUAP, clicando em autorizar o aplicativo.
  5. O SUAP processa as credenciais e redireciona de volta para a aplicação com o código de autorização (`code`).
  6. O backend realiza a troca do código e sincroniza o perfil do usuário localmente.
  7. O usuário é autenticado e redirecionado automaticamente para o seu respectivo Dashboard.
* **Critérios de Aceitação:**
  - A autenticação deve ser realizada exclusivamente via SUAP (OAuth2).
  - Senhas locais não podem ser solicitadas ou salvas no banco de dados.
  - A sessão deve ser gerenciada com cookies HttpOnly seguros.
  - Caso ocorra falha de comunicação com o SUAP, a interface deve exibir um Toast descritivo sobre o erro.
  - Se for o primeiro login do usuário, uma conta deve ser criada localmente de forma automática.

### US002 — Cadastro e Gestão de Projetos (Orientador)
* **Como** Professor Orientador do IFAL
* **Eu quero** cadastrar e gerenciar meus projetos acadêmicos e suas equipes
* **Para que** eu possa estruturar a colaboração com meus orientandos.
* **Fluxo de Usuário:**
  1. O orientador autenticado entra na seção de "Projetos".
  2. Clica no botão "Novo Projeto".
  3. Preenche os campos obrigatórios: Título, Descrição, URL do Repositório Git (opcional) e seleciona os Alunos que farão parte da equipe.
  4. Clica em "Salvar".
  5. O sistema grava o projeto e o exibe no painel do orientador e nas telas dos alunos adicionados.
* **Critérios de Aceitação:**
  - Apenas usuários com perfil `advisor` (ou superior) podem criar, editar ou remover projetos acadêmicos.
  - É obrigatório informar Título, Descrição e no mínimo 1 Aluno participante.
  - A exclusão ou edição deve auditar a operação registrando o autor no banco de dados.

### US003 — Gerenciamento de Quadro Kanban (Aluno)
* **Como** Aluno orientando membro de um projeto
* **Eu quero** gerenciar minhas tarefas e prazos em um quadro Kanban visual
* **Para que** eu possa organizar e acompanhar o andamento das minhas atividades semanais.
* **Fluxo de Usuário:**
  1. O aluno acessa os detalhes do seu respectivo Projeto.
  2. Clica na aba "Quadro Kanban".
  3. Visualiza as colunas: "A Fazer", "Em Progresso" e "Concluído".
  4. Cria uma nova tarefa inserindo Título, Descrição, Prazo e define qual membro é o Responsável.
  5. Move as tarefas entre colunas arrastando-as ou usando botões de movimentação rápida de status.
* **Critérios de Aceitação:**
  - Alunos e Orientadores vinculados ao projeto específico podem adicionar, editar ou mudar o status de tarefas.
  - A movimentação de colunas deve atualizar o status no banco de dados em tempo real.
  - Tarefas cuja data limite seja menor do que a data atual devem exibir um sinalizador de "Atrasada" de forma destacada em vermelho.

### US004 — Submissão de Entregas e Artefatos (Aluno)
* **Como** Aluno participante de um projeto
* **Eu quero** realizar o upload de arquivos e documentos de entrega na plataforma
* **Para que** meu orientador possa avaliar o progresso das entregas acadêmicas.
* **Fluxo de Usuário:**
  1. O aluno acessa a página de "Submissões" do seu projeto.
  2. Visualiza a lista de entregas anteriores e o status atual.
  3. Clica em "Fazer Nova Submissão", seleciona o arquivo PDF/ZIP e envia.
  4. O sistema processa o upload, gera uma versão incremental automática (ex: v1 -> v2) e atualiza o histórico de submissões.
* **Critérios de Aceitação:**
  - O tamanho limite de cada arquivo submetido é de 50 MB.
  - Cada nova entrega deve gerar um número de versão incremental automática para manter a rastreabilidade.
  - O sistema deve persistir e exibir no histórico o nome do autor da submissão, data e horário do upload.

### US005 — Avaliação e Feedback de Entregas (Orientador)
* **Como** Professor Orientador do projeto acadêmico
* **Eu quero** avaliar as submissões de arquivos enviadas pelos alunos e anexar um feedback
* **Para que** os discentes saibam o que precisa ser corrigido e refinado no projeto.
* **Fluxo de Usuário:**
  1. O orientador accesses a aba "Submissões" do projeto que está avaliando.
  2. Visualiza a lista de arquivos e baixa a versão mais recente submetida pela equipe.
  3. Clica no botão "Avaliar Entrega".
  4. Digita o feedback textual e altera o status para "Avaliado/Aprovado" ou "Pendente/Necessita Ajustes".
  5. Salva a avaliação. O aluno passa a visualizar o feedback em seu dashboard em tempo real.
* **Critérios de Aceitação:**
  - Apenas o orientador responsável pelo projeto e os administradores podem avaliar e enviar feedbacks.
  - O feedback em texto é um campo obrigatório no formulário de avaliação.
  - O status da submissão e o feedback devem ser atualizados imediatamente na interface do aluno.